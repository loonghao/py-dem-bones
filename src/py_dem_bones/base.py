        ...
        if transformations.shape[0] == 0:
            ...
            for b in range(self.num_bones):
                if b < len(self._bind_matrices):
                    ...
                    flat_transforms[b*3:(b+1)*3, :] = self._bind_matrices[b][:3, :]
                else:
                    # 对于没有绑定矩阵的骨骼，使用单位矩阵
    ...
    def set_weights(self, weights):
        ...
        if not isinstance(weights, np.ndarray):
            try:
                ...
                raise ParameterError(f"Failed to convert weights to numpy array: {str(e)}")
        
        # Check dimensions
        if len(weights.shape) != 2:
            raise ParameterError(f"Weights must be a 2D array, got shape {weights.shape}")
    ...
    def set_rest_pose(self, vertices):
        ...
        if not isinstance(vertices, np.ndarray):
            try:
                ...
                raise ParameterError(f"Failed to convert vertices to numpy array: {str(e)}")
    ...
    def set_transformations(self, transformations):
        ...
        if not isinstance(transformations, np.ndarray):
            try:
                ...
                raise ParameterError(f"Failed to convert transformations to numpy array: {str(e)}")
        
        # Check dimensions
        if len(transformations.shape) != 3 or transformations.shape[1:] != (4, 4):
            raise ParameterError(f"Transformations must have shape [num_frames, 4, 4], got {transformations.shape}")
